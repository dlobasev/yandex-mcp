...
- **Объявления**: получение, создание (с изображениями, сайтлинками, уточнениями), модерация, управление состоянием
- **Сайтлинки**: создание наборов, получение, удаление
- **Уточнения (Callouts)**: создание, получение, удаление
...
| `direct_create_sitelink_set` | Создать набор быстрых ссылок (1-8 шт) |
| `direct_get_sitelink_sets` | Получить наборы сайтлинков |
| `direct_delete_sitelink_sets` | Удалить наборы сайтлинков |
| `direct_create_callouts` | Создать уточнения (callouts) |
| `direct_get_callouts` | Получить уточнения |
| `direct_delete_callouts` | Удалить уточнения |
...
## Известные ограничения Yandex Direct API

- **Сайтлинки и уточнения** — API v5/v501 поддерживает привязку только на уровне объявления (`TextAd.SitelinkSetId`, `TextAd.AdExtensionIds`). Кампанийный и групповой уровни с наследованием доступны только в веб-интерфейсе Директ Про.
- **Промоакции** — в API нет метода для создания и управления промоакциями (промо-расширениями объявлений).
